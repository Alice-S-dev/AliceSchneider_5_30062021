...
const chiffrePanier = document.getElementById("chiffre-panier"); // On récupère l'élément html où sera affiché la quantité
if (contenuStorage !== null) {
	...
	for(let produit in contenuStorage) { //pour chaque produit présent dans le LS
		...
		const qteProduitsPanier = contenuStorage[produit].quantite; // On récupère la quantité
	...
	const resultat = (qteTableau.reduce(reducteur, 0)); // On ajoute une valeur initiale de 0 pour eviter l'erreur si le tableau est vide
